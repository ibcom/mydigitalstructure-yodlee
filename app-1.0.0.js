...
exports.handler = function (event, context)
{
			...
			source:
			{
				...
				transactions: function (options, response)
				{
					...
					else
					{
						...
						_.each(app.data.source.transactions, function (transaction) {transaction.transactionDate = transaction.date});
					...
					send: function (options, response)
							{
								...
								if (_.isObject(app.data.source.processTransaction))
								{
									...
									//??? check baseType for amount - myds type (1=Credit, 2=Debit)
									...
									data.type = 1 //Credit
									if (_.upperCase(trasaction.baseType) == 'DEBIT')
									{
										data.type = 2
									}
	...
	app._util.show = 
	{
		...
		transactions: function (options)
		{
			...
			var transactions = app.data.source.transactions
