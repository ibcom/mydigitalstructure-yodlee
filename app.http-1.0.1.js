...
var app = {_util: {}, data: {source: {}, destination: {}, event: event}, http: {port: 3000}, version: '1.0.1'};
					...
					{	
						...
						"param":
						{
							"user":
							{
					  			...
					  			"email": 'yodlee-' + access.space + '@mydigitalstructure.com'
					  		},
					  		"preferences":
					  		{
								"currency": "AUD",
					...
					{	
						...
						if (response.errorCode != undefined)
						{
							if (options.httpResponse != undefined) {app.http.requestHandlerEnd(options.httpResponse,
										{status: 'ER', errorMessage: response.errorMessage, errorCode: response.errorCode})}
						}
						else
						{
							app.data.yodlee.user = JSON.parse(data).user;
							mydigitalstructure._util.testing.data(JSON.parse(data), 'response');
